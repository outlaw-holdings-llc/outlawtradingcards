...
// POST /api/internal/break-claim — DO charges a buyer their saved card when they
// claim a break spot (fixed price). Bearer AUCTION_SECRET. Returns paid/failed;
// the DO only marks the spot claimed on 'paid'.
export const onRequestPost = route(async ({ request, env }) => {
  ...
  const label = String(b.spot_label || 'Spot');
  const breakTitle = String(b.break_title || 'Live Break');
  if (!userId || amount < 50) throw err('Invalid claim', 400);

  const user = await env.DB.prepare('SELECT id, stripe_customer_id FROM users WHERE id = ?').bind(userId).first();
  if (!user || !user.stripe_customer_id) return json({ status: 'failed', error: 'no card on file' });
  ...
  const orderId = uuid();
  const items = JSON.stringify([{ title: `${breakTitle} — ${label}`, break: breakTitle, spot: label, price_cents: amount }]);
  await env.DB.prepare(`INSERT INTO orders (id,user_id,amount_cents,status,items) VALUES (?,?,?,?,?)`)
  ...
  try {
    pi = await s.post('/payment_intents', {
      ...
      metadata: { order_id: orderId, user_id: userId, kind: 'break', spot: label },
    }, orderId);
  } catch (e) {
